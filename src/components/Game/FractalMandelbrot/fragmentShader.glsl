 precision highp float;
    ...
    int mandelbrot(vec2 c) {
      vec2 z = vec2(0.0, 0.0);
      int iterations = 0;
      const int maxIterations = 10000;  // Увеличили количество итераций
    ...
    // Функция для плавного вычисления цвета
    vec3 colorScheme(int iterations) {
      float t = float(iterations) / 10000.0;

      if (iterations == 10000) {
          return vec3(0.0, 0.0, 0.0);  // Черный цвет для точек, не покидающих фрактал
      }

      // Переход от черного к ярким цветам
